<?php
    ...
    // If it's 'complete' but amount is less now — reset to 'pending'
    ...
    if(isset($_POST['AddCustomerQuery']))
    {
        ...
        try
        {
            ...
            // $stmt = $mysqli->prepare("INSERT into query_det (mst_id,remarks,call_time,call_date,user_id,status)values (?,?,?,?,?,?)");
            // $stmt->bind_param("isssis", $mst_id, $_POST['remarks'], $_POST['call_time'], $_POST['call_date'], $user['id'], $_POST['status']);
            // $stmt->execute();
    ...
    if (isset($_POST['btnSave'])) 
    {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            alert("Save Successfully", "success", "success");
            redirect("query.php?status=converted");
        } 
        catch (Exception $e) 
        {
            alert($e->getMessage(), "error", "error");
    ...
    if (isset($_POST['ShareLeadAdd'])) 
    {
        ...
        // Always load existing list (lead creator always present as first ID)
        ...
        // Merge new shared users (avoiding duplicates)
    ...
    if (isset($_POST['leadAllocate'])) 
    {
        ...
        foreach ($leadIds as $leadId) {
         ...
         $_SESSION['alert'] = [
                ...
                'text' => 'Leads successfully Transfer.',
                'icon' => 'success'
            ];
...
.leadCheckbox {
    ...
    cursor: pointer;
    accent-color: #4caf50;
    /* Modern browsers support this for checkbox color */
    margin: 5px;
}
...
<div class="content-body">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <button type="button" class="btn btn-square btn-outline-danger" id="add-query">Add
                            Query</button>
                    ...
                    <div class="query-button m-4 flex-wrap">
                        ...
                        <a href="?status=<?php echo $row['name'] ?>"
                            class="btn mt-md-0 mt-2 <?php if ($_GET['status'] == $row['name']) echo 'btn-success'; else echo 'btn-primary'; ?>"><?= $row['name'] ?>
                        </a>
                        <?php }  ?>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="" method="Post">
                            ...
                            <div class="row m-3">
                                <div class="col-md-3">
                                    <select name="lead_assign_user" id="" class="form-control select2" required>
                                        <?php
                                    // $user_id = $row['user_id'];
                                    if($_SESSION['user'] == "admin" ){
                                        ...
                                        $stmt_user = $mysqli->prepare("SELECT * FROM users WHERE id = ? OR parent_id = ?");
                                ...
                                <div class="col-md-2">
                                    <button type="submit" class="btn btn-outline-success" name="leadAllocate">Submit</button>
                                </div>
                            </div>
                            <?php endif;  ?>
    ...
    if (isset($_GET['status']) && $_GET['status'] == "All") {
        $stmt = $mysqli->prepare("SELECT a.*, b.name as customer, c.name as user, d.name as destination_name
            FROM query_mst a
    ...
    if (isset($_GET['status']) && $_GET['status'] == "All") {
        ...
        $stmt = $mysqli->prepare("SELECT a.*, b.name as customer, c.name as user, d.name as destination_name
            ...
            ORDER BY a.pinned DESC, a.id DESC");
        $stmt->bind_param('sii', $_GET['status'], $_SESSION['id'], $_SESSION['id']);
    }
} else {
    if (isset($_GET['status']) && $_GET['status'] == "All") {
        $stmt = $mysqli->prepare("SELECT a.*, b.name as customer, c.name as user, d.name as destination_name
        ...
        $stmt = $mysqli->prepare("SELECT a.*, b.name as customer, c.name as user, d.name as destination_name
            ...
            AND FIND_IN_SET(?, a.user_id)
            ORDER BY a.pinned DESC, a.id DESC");
        $stmt->bind_param('si', $_GET['status'], $_SESSION['id']);
    }
}
 ...
 ?>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <br>
                                                <?php if($_SESSION['user'] == "admin" || $_SESSION['user'] == "Team Manager"): ?>
                                                <input type="checkbox" class="leadCheckbox" value="<?= $row['id'] ?>"
                                                    name="lead_id[]">
                                                <?php endif; ?>
                                                <br>
                                                <?php if($row['pinned']): ?>
                                                ...
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    <div class="dropdown m-2">
                                                        ...
                                                        <ul class="dropdown-menu"
                                                            ...
                                                            <li>
                                                                <?php if($row['pinned'] == 0): ?>
                                                                <button  type="button" class="dropdown-item pin-query"
                                                                    data-id="<?= $row['id'] ?>" data-value="1">📌
                                                                    Pin</button>
                                                                <?php else: ?>
                                                                <button  type="button" class="dropdown-item pin-query"
                                                                    data-id="<?= $row['id'] ?>" data-value="0">❌
                                                                    Unpin</button>
                                                                <?php endif; ?>
                                                            </li>

                                                            <li>
                                                                <button type="button" title="Edit Query" class="dropdown-item edit"
                                                                    data-id="<?= $row['id'] ?>">
                                                                    Edit Query
                                                                </button>
                                                            </li>
                                                            ...
                                                            <li>
                                                                <button  type="button" class="dropdown-item ShowRemarks"
                                                                    data-id="<?= $row['id'] ?>">
                                                                    Show Remarks
                                                                </button>
                                                            </li>
                                                            ...
                                                            <li>
                                                                <a href="add_quote.php?id=<?= $row['customer_id'] ?>&q_id=<?= $row['id']; ?>"
                                                                    title="Add Quote" class="dropdown-item">
                                                                    Create Quote
                                                                </a>
                                                            </li>
                                                            ...
                                                            <li>
                                                                <a
                                                                    href="created_quote.php?q_id=<?= $row['id'] ?>&id=<?= $row['customer_id'] ?>">
                                                                    <button type="button" class="dropdown-item">Created
                                                                        Quote</button>
                                                                </a>
                                                            </li>
                                                            ...
                                                            <li>
                                                                <button  type="button" class="dropdown-item" id="addSaleAmount"
                                                            ...
                                                            <li>
                                                                <button type="button" class="dropdown-item"
                                                                    ...
                                                                    id="add_expenses">Expenses</button>
                                                            </li>
                                                            <?php endif; ?>
                                                            ...
                                                            <li>
                                                                <button type="button" id="payment_button"
                                                                    data-bs-toggle="modal"
                                                                    data-bs-target="#payment_collect"
                                                                    data-id="<?= $row['id'] ?>"
                                                                    class="dropdown-item">Payment Collect</button>
                                                            </li>
                                                            <li>
                                                                <button type="button"
                                                                    class="dropdown-item shareLeadButton"
                                                                    data-bs-toggle="modal" data-id="<?= $row['id'];  ?>"
                                                                    data-bs-target="#shareLead">
                                                                    Share Lead
                                                                </button>
                                                            </li>
                                                        </ul>
