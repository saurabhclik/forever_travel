...
if (isset($_POST['btnadd'])) {
    ...
    if (!empty($_POST['id'])) {
        try {
            $stmt = $mysqli->prepare("UPDATE expense_category set name=?,active=? WHERE id=?");
            $stmt->bind_param("sii", $_POST['name'], $_POST['active'], $_POST['id']);
            $stmt->execute();
            $stmt->close();
        ...
        try {
            ...
            $stmt = $mysqli->prepare("INSERT into expense_category (name,active)values (?,?)");
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="example" class="display" style="min-width: 845px">
                                ...
                                <tbody>
                                    <?php
                        $stmt = $mysqli->prepare("SELECT * from  expense_category  order by id desc");
                        $stmt->execute();
                        $res = $stmt->get_result();
                        $sno = 1;
        ...
        <div class="modal-dialog  modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title fs-5" id="exampleModalLabel"> <span class="btnname"> Add Expense Category</span></h3>
...
<script>
   $(document).on("click", ".edit", function() {
    console.log('hello');
        ...
        $(".btnname").html('Update Expense Category');
